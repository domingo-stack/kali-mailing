...
export default function ProjectsPage() {
  ...
  const fetchProjects = async () => {
    ...
    if (teamError || !teamData) {
      console.error('Error al buscar equipo, o el usuario no pertenece a ninguno:', teamError);
      setProjects([]);
      setLoading(false);
      return;
    }
    ...
    const { data, error } = await supabase.from('projects').select('*').eq('team_id', currentTeamId).order('created_at', { ascending: false });
  ...
  const handleAddProject = async (e: FormEvent) => {
    ...
    if (error) console.error('Error adding project via RPC:', error);
    else {
      setNewName('');
      setNewDescription('');
      await fetchProjects();
      ...
      <main className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <form onSubmit={handleAddProject} className="mb-8 p-4 bg-white rounded-lg shadow-sm border border-gray-200 space-y-4">
          ...
          <div>
            ...
            <input id="projectName" type="text" value={newName} onChange={(e) => setNewName(e.target.value)} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm" placeholder="Ej: Lanzamiento Q4" required />
          </div>
          <div>
            <label htmlFor="projectDescription" className="block text-sm font-medium text-gray-700">Descripción (Opcional)</label>
            <textarea id="projectDescription" value={newDescription} onChange={(e) => setNewDescription(e.target.value)} rows={3} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm" placeholder="Describe brevemente el objetivo del proyecto"></textarea>
          </div>
          <div className="text-right">
            <button type="submit" className="px-4 py-2 bg-blue-600 text-white font-semibold rounded-md shadow-sm hover:bg-blue-700">Crear Proyecto</button>
          </div>
        </form>
